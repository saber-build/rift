...
- Created rifts must not be stored inside the workspace being copied, because a copy would otherwise recursively contain existing rifts (`InsideSource`). The one exception is a filtered copy whose patterns exclude the destination's workspace-relative path or a directory above it: the walk never descends into an excluded directory, so the copy cannot contain the destination or its siblings (nor the adjacent `.trash`). An exact copy — `copyAll`, or a filter that drops nothing — applies no patterns and is always refused inside the workspace. The check runs before anything is created under `into`, and again on the canonical destination.
